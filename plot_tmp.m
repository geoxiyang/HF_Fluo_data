...
load([datapath,'SIF760daily.mat'],'halfhourly_result','SIF_mean');
...
%hold on
plot(sif_gosat_local(sub5),nanmean(gpp_cube(sub5,26:28),2),'m.','MarkerSize',24)
hold on

plot(sif_gome_local(sub4),nanmean(gpp_cube(sub4,18:20),2),'g.','MarkerSize',24)
...
[b,bint,r,rint,stats] = regress(nanmean(gpp_cube(sub5,26:28),2),x);
yfit = b(1) + b(2) * xfit;
plot(xfit,yfit,'m-','LineWidth',2);
pos2 = [0.1,38];
text(pos2(1),pos2(2),['GPP=',num2str(b(1),'% 5.2f'),'+',num2str(b(2),'% 5.2f'),'\times','SIF',...
                      '  ',...
                      'r^{2}=',num2str(corr(nanmean(gpp_cube(sub5,26:28),2),sif_gosat_local(sub5))^2,'% 5.2f'),...
                      '  ',...
...
[b,bint,r,rint,stats] = regress(nanmean(gpp_cube(sub4,18:20),2),x);
yfit = b(1) + b(2) * xfit;
plot(xfit,yfit,'g-','LineWidth',2);
pos3 = [0.1,35];
text(pos3(1),pos3(2),['GPP=',num2str(b(1),'% 5.2f'),'+',num2str(b(2),'% 5.2f'),'\times','SIF',...
                      '  ',...
                      'r^{2}=',num2str(corr(nanmean(gpp_cube(sub4,18:20),2),sif_gome_local(sub4))^2,'% 5.2f'),...
                      '  ',...
...
ylabel('GPP(umol CO_{2}/m^{2}/second)','FontName','Whitney','FontSize',24);
...
print(gcf, '-dpng','-r300', '/Users/xiyang/Dropbox/Mypaper/6.2014-Fluorescence/resub2/figures/SIF_GPP_multiple_times_2.png');
close(gcf);
